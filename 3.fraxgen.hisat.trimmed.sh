...
OUTPUT=$SCRATCH/FRAXGEN/3.MAPPING.HISAT2
...
hisat2 -p 96 --score-min L,0,-0.6 -x $REF/Fexcelsior.indexed.genome -1 $INPUT/${sample}_R1_p.fastq.gz -2 $INPUT/${sample}_R2_p.fastq.gz -S $OUTPUT/${sample}_ht2.sam --summary-file $OUTPUT/${sample}_summary.txt --un-gz $OUTPUT/OTHER_FILES/ --al-gz $OUTPUT/OTHER_FILES/ --un-conc-gz $OUTPUT/OTHER_FILES/ --al-conc-gz $OUTPUT/OTHER_FILES/

# Convert SAM to BAM
samtools view -S -b $OUTPUT/${sample}_ht2.sam > $OUTPUT/${sample}_ht2.bam
# SORT -n sorts bam by name
samtools sort -l 1 -@96 -n -o $OUTPUT/${sample}_ht2.nsrt.bam -T $OUTPUT/tmp/${sample}_tmp $OUTPUT/${sample}_ht2.bam # Sort SAM by name and save in BAM
...
samtools sort -l 1 -@96 -o $OUTPUT/${sample}_ht2.fix.psrt.bam -T $OUTPUT/tmp/${sample}_tmp $OUTPUT/${sample}_ht2.nsrt.fix.bam # Sort SAM by chr position and save in BAM
